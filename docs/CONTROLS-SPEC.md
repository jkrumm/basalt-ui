...
| `RangeFilter`       | `{ field: FieldHandle<RangeField<P>>; icon?: ReactNode; customPicker?: ComponentType<RangeCustomPickerProps>; label?: string /* heading + aria name, default 'Range' */ }` | `field.range` — three URL params (`window`/`from`/`to`, renamable)                                                                                                                                                                                                        | sheet: presets as vertical `SegmentedControl` (>4) + the custom picker                                                                                                                                                                                      |
