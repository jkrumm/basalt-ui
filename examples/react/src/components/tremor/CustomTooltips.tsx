import type { TooltipProps } from './BarChart.tsx'
import type { TooltipProps as TooltipComboBarChartProps } from './ComboChart.tsx'
import { chartColors, getColorClassName } from '@/lib/chartUtils.ts'
import { cx, formatters } from '@/lib/utils.ts'
      ...
      <div className="space-y-2">
        {payload.map((category, index) => (
          <div key={index} className="flex space-x-2.5">
            <span
              className={cx(getColorClassName(category.color, 'bg'), 'w-1 rounded')}
              aria-hidden="true"
            />
