...
import { Drawer, DrawerClose, DrawerContent, DrawerTitle } from '@/components/tremor/Drawer.tsx'
import { useIsMobile } from '@/lib/useMobile'
import { cx, focusRing } from '@/lib/utils.ts'
import { sidebarOpen } from '@/stores/sidebar.ts'
...
>(
  (
    ...
    const storeOpen = useStore(sidebarOpen)
    ...
    return (
      <SidebarContext.Provider value={contextValue}>
        <div style={style} className={cx('flex min-h-svh w-full', className)} ref={ref} {...props}>
          {children}
        </div>
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={cx(
            'relative bg-transparent transition-[width] duration-150 ease-in-out will-change-transform',
            'group-data-[collapsible=true]:w-0',
          )}
          style={{
            width: state === 'collapsed' ? '0' : SIDEBAR_WIDTH,
            height: '100%',
          }}
        />
        <div
          className={cx(
            'fixed z-10 hidden transition-[left,right,width] duration-150 ease-in-out will-change-transform md:flex',
            'border-r border-gray-200 dark:border-gray-800',
            className,
          )}
          style={{
            width: SIDEBAR_WIDTH,
            left: state === 'collapsed' ? `-${SIDEBAR_WIDTH}` : '0',
            top: '0',
            bottom: '0',
            height: '100%',
          }}
...
>(({ children, isActive, icon, notifications, className, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {notifications && (
        <span className="inline-flex size-5 items-center justify-center rounded bg-blue-100 text-sm font-medium text-gray-50 sm:text-xs dark:bg-blue-500/10 dark:text-blue-200">
          {notifications}
        </span>
      )}
    </a>
