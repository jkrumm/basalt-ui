...
| `./content`         | coupled  | The content/prose surface (`docs/CONTENT-SPEC.md`): `Prose` (article/chat typography), `CodeBlock` (shiki, optional peer, lazy singleton), `Callout`, `TableOfContents` (scroll-spy), `ReadingProgress`, `headingSlug`/`SlugTracker`/`readingTime`, `Markdown` (react-markdown + remark-gfm, optional peers, streaming-aware AI-output renderer), `MermaidDiagram` (beautiful-mermaid, optional peer), `mdxComponents`/`createMdxComponents` (MDX runtime element map), `blockSplit`, `ArticleLayout` (docs-page frame: meta header + sticky TOC rail + prev/next footer), `ArticleCard`/`ArticleGrid` (docs-landing overview cards), Article model (`sortArticles`/`filterArticles`/`formatArticleDate`), `FilterSet`/`ViewTabs`/`MultiSelectFilter` re-exported from `./controls` (replaced the controlled `ArticleFilterBar`), `toArticleActions` (Spotlight projector), `GuideLink`/`GuideDrawer` (contextual in-app help drawer)                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `./controls`        | coupled  | The control tier (`docs/CONTROLS-SPEC.md` §3): `FilterSet`, `RangeFilter`/`CompareFilter`/`SelectFilter`/`MultiSelectFilter`/`SearchFilter`/`ToggleFilter` (each bound to a `FieldHandle`), `ViewTabs`, and the action/sync family (`ActionGroup`, `OverflowMenu`, `SyncButton`, `BarAction`/`GlobalAction`). Resolves with no `@mantine/dates` installed — the date picker is injected via `RangeFilter.customPicker`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `./controls-dates`  | coupled  | `DateRangePicker` — the `@mantine/dates` implementation of `RangeFilter`'s `customPicker` seam; `@mantine/dates` is an optional peer only this subpath needs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`BasaltShell` composes `AppSidebar` / `MobileNav` / `AppBreadcrumbs` / `PageBar` (row 1 portals
into the 48px header via an internal provider/outlet, row 2 in-flow sticky publishing
`--basalt-page-bar-h`). Brand, `SidebarSection[]`, `globalActions: GlobalAction[]` (rendered in
the header and folded into the shared mobile kebab), settings/account extras; collapse persisted via basalt's own
...
Mechanically: add the kind to `GRACE_PERIOD_KINDS` in `src/guard/index.ts`, as a `{ since, promote,
why }` entry (semver strings — C16, `docs/CONTROLS-SPEC.md` §1), in the same commit that ships it;
deleting that entry one minor later IS the promotion, and belongs in its own commit so the
changelog reads "enforcement got stricter". **The promise is machine-checked, not just written
down**: `src/guard/grace.test.ts` fails the build once `package.json`'s version reaches an entry's
`promote` while the entry is still in the ledger, and asserts `since` precedes `promote` for every
entry. Before this existed, a bare promotion-note string carried no expiry anything checked, and
five kinds landed together in the round-4 guard minor sat at `warn` for five minors as a result (D4,
`docs/archive/CONTROLS-SYNTHESIS.md`) — promoted in the Unreleased minor, see `MIGRATING.md`.
...
file's top-level keys are fixed by oxlint's parser), entries shaped `{ since, promote, why }` same
as `GRACE_PERIOD_KINDS`. A test asserts it against the shipped preset in BOTH directions, so
deleting an entry forces the level flip in the same commit, plus the C16 version gate
(`oxlint-plugin.test.ts`) that fails the build once `package.json` reaches an entry's `promote`
while it's still there — a rule cannot sit at `warn` for twelve minors with nothing tracking it
again. Read the promotion state there; never restate the list in prose. A new plugin rule ships
`"warn"` in `configs/oxlint.json` (the consumer preset) and `"error"` in the repo-local
`.oxlintrc.json` — basalt fixes its own violations immediately, consumers get a minor's runway.
Promotion is flipping the shipped level, in its own commit. Corollary, and the reason
...
**A rule that may never promote belongs in `PLUGIN_RULE_ADVISORY`, not `PLUGIN_RULE_GRACE`.**
Shaped `{ since, why }` — no `promote`, and the C16 gate skips it outright. `shadow-basalt-export`
is the one entry: a name collision is evidence a component was forked, not proof, and renaming the
fork defeats the check, so there is no version at which promising `error` in advance is honest. It
still ships `warn` in the shipped preset and needs a written `why`, same bar as a grace entry —
ADVISORY changes where the entry lives and whether it's ever expected to move, not how seriously
it's tracked.
