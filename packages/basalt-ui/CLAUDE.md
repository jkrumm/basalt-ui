...
`shadow-basalt-export` (all nine barrels), restarting grace for both. 1.21.1 NARROWED
...
each can judge.** They have disagreed four times in three rounds — 1.21.1 fixed the last two (a
one-line `/** theme-allow … */` docblock opener, and a `{/*` whose token wrapped onto the next line,
which left a bare `}` after comment-stripping and scoped the waiver to a brace). The full
thirteen-shape matrix is pinned in BOTH `src/guard/check-source.test.ts` and
`configs/oxlint-plugin.test.ts`; add a shape to one and it goes in the other in the same commit. The three boundary rules above deliberately do **not** — an architecture boundary a stray
