...
export function generateLlmsTxt(): string {
  const lines: string[] = [
    ...
    '# Every peer is optional: ./tokens, ./charts, ./state and ./guard resolve with',
    '# no React and no Mantine installed. A consumer with no React at all can take the',
    '# token system alone — `bunx basalt-ui tokens:css --out tokens.css`, or the prebuilt',
    '# ./tokens.css subpath. Guide: docs/FRAMEWORK-FREE.md in the repo.',
    '',
    `Package: ${pkg.name}`,
    `Version: ${pkg.version}`,
    '',
