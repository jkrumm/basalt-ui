...
/**
 * The four boxes a consumer can reach (`common/props.ts`): `root` is the header stack, `title` is
 * the heading ELEMENT (`<h2>`/`<h3>`) rather than its row, `metric` is the hero `value`/`unit`/
 * `delta` row, `icon` is the leading `IconSlot`.
 *
 * The other boxes this paints — the title row, the count tag, the actions span, the subtitle, the
 * sparkline, the info bubble — are deliberately absent. Each is a one-shot readout basalt positions,
 * not a layout box a consumer places, and `actions`/`sparkline` already take a `ReactNode` the
 * caller can class itself.
 */
export type WidgetHeaderSlot = 'root' | 'title' | 'metric' | 'icon'

export type WidgetHeaderProps = BasaltProps &
  SlotStylesProps<WidgetHeaderSlot> & {
  ...
  return (
    ...
    >
      <div className={classes.titleRow}>
        <Heading className={cx(classes.heading, classNames?.title)}>
          ...
          {icon !== undefined && (
            <IconSlot className={cx(classes.icon, classNames?.icon)}>{icon}</IconSlot>
          )}
